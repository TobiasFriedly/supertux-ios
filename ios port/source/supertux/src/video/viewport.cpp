...
inline float
calculate_auto_scale(const Size& min_size, const Size& max_size,
                     const Size& window_size)
{
  ...
  {
    const float window_aspect = static_cast<float>(window_size.width) /
                                static_cast<float>(window_size.height);
    const float max_aspect = static_cast<float>(max_size.width) /
                             static_cast<float>(max_size.height);

    if (window_aspect > max_aspect)
      scale = std::min(scale, static_cast<float>(window_size.height) / static_cast<float>(max_size.height));
  }

  if (g_config->mobile_controls && window_size.height >= min_size.height)
...
{
#ifdef SUPERTUX_IOS
  const float base_scale = calculate_auto_scale(min_size, max_size, window_size);
  const float zoom_multiplier = magnification > 0.f ? std::max(1.f, magnification) : 1.f;
  return base_scale * zoom_multiplier;
#else
  float scale = magnification;
  if (scale == 0.0f) // magic value
    scale = calculate_auto_scale(min_size, max_size, window_size);
