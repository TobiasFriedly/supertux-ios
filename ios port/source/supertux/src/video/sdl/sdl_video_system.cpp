...
SDLVideoSystem::get_window_size() const
{
  ...
  Size output_size;
  if (m_sdl_renderer &&
      SDL_GetRendererOutputSize(m_sdl_renderer.get(), &output_size.width, &output_size.height) == 0 &&
  ...
  return SDLBaseVideoSystem::get_window_size();
}
...
#else
    Size target_size = get_window_size();
#endif
    if (target_size.width <= 0 || target_size.height <= 0)
      target_size = m_desktop_size;
#ifdef SUPERTUX_MOBILE
    m_viewport = Viewport::from_size(target_size, target_size);
#else
