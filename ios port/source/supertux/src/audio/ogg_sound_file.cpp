...
bool
OggSoundFile::seek_time(double seconds)
{
  if (seconds < 0.0)
    seconds = 0.0;

  return ov_time_seek(&m_vorbis_file, seconds) == 0;
}

double
OggSoundFile::get_duration() const
{
  return ov_time_total(const_cast<OggVorbis_File*>(&m_vorbis_file), -1);
}
