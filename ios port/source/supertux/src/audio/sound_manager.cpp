...
SoundManager::enable_music(bool enable)
{
  ...
  if (m_music_enabled) {
    play_music(m_current_music, 0.f, m_music_looping);
  } else {
    if (m_music_source) {
      m_music_source.reset();
    }
...
{
  play_music(filename, fadetime, true);
}

void
SoundManager::play_music(const std::string& filename, float fadetime, bool looping)
{
  if (filename == m_current_music && m_music_source != nullptr)
  {
    if (m_music_looping != looping)
    {
      m_music_source.reset();
    }
    else if (!m_music_source->playing() && !m_music_source->paused())
    {
      m_music_source->seek_time(0.0);
      m_music_source->play();
      return;
    }
    else
...
bool
SoundManager::seek_music(double seconds)
{
  return m_music_source != nullptr && m_music_source->seek_time(seconds);
}

double
SoundManager::get_music_position() const
{
  return m_music_source ? m_music_source->get_position() : 0.0;
}

double
SoundManager::get_music_duration() const
{
  return m_music_source ? m_music_source->get_duration() : 0.0;
}

bool
SoundManager::is_music_finished() const
{
  return m_music_source != nullptr && m_music_source->finished();
}

void
SoundManager::recover_audio_context()
{
  if (m_context == nullptr)
    return;
...
#endif
  alcMakeContextCurrent(m_context);
  check_alc_error("Couldn't restore audio context: ");
  alcProcessContext(m_context);
  check_alc_error("Couldn't process restored audio context: ");

  if (m_music_source && !m_music_source->paused() && !m_music_source->playing())
    m_music_source->play();
}
...
{
#ifdef SUPERTUX_IOS
  if (supertux_ios_audio_interruption_ended())
  {
    try
    {
      recover_audio_context();
    }
    catch (const std::exception& err)
    {
      log_warning << "Failed to recover iOS audio context: " << err.what() << std::endl;
    }
  }
#endif
