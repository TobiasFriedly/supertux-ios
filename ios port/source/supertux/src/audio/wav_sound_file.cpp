...
bool
WavSoundFile::seek_time(double seconds)
{
  if (seconds < 0.0)
    seconds = 0.0;

  const size_t frame_size = static_cast<size_t>(std::max(1, m_channels)) *
                            static_cast<size_t>(std::max(1, m_bits_per_sample / 8));
  const double bytes_per_second = static_cast<double>(std::max(1, m_rate)) *
                                  static_cast<double>(frame_size);
  size_t byte_offset = static_cast<size_t>(seconds * bytes_per_second);
  byte_offset -= byte_offset % frame_size;
  byte_offset = std::min(byte_offset, m_size);

  return PHYSFS_seek(m_file, static_cast<PHYSFS_uint64>(m_datastart + byte_offset)) != 0;
}
