...
EditorLayersWidget::draw(DrawingContext& context)
{
  ...
  context.color().draw_filled_rect(Rectf(Vector(static_cast<float>(m_Left), static_cast<float>(m_Ypos)),
                                         Vector(static_cast<float>(m_Right), static_cast<float>(m_Ypos + m_RowHeight))),
  ...
  switch (m_hovered_item)
  {
    case HoveredItem::SPAWNPOINTS:
      target_rect = Rectf(Vector(static_cast<float>(m_Left), static_cast<float>(m_Ypos)),
                          Vector(static_cast<float>(m_Xpos), static_cast<float>(m_Ypos + m_RowHeight)));
      break;

    case HoveredItem::SECTOR:
      target_rect = Rectf(Vector(static_cast<float>(m_Xpos), static_cast<float>(m_Ypos)),
                          Vector(static_cast<float>(m_sector_text_width + m_Xpos), static_cast<float>(m_Ypos + m_RowHeight)));
      break;

    case HoveredItem::LAYERS:
      {
        Vector coords = get_layer_coords(m_hovered_layer);
        target_rect = Rectf(Vector(coords.x, static_cast<float>(m_Ypos)),
                            Vector(coords.x + static_cast<float>(m_RowHeight),
                                   static_cast<float>(m_Ypos + m_RowHeight)));
      }
      break;
  ...
  context.color().draw_text(Resources::normal_font, m_sector_text,
                            Vector(static_cast<float>(m_Xpos) + 3.0f, static_cast<float>(m_Ypos) + 5.0f),
  ...
  int pos = 0;
  const float icon_size = static_cast<float>(m_RowHeight);
  for (const auto& layer_icon : m_layer_icons)
  {
    if (!layer_icon->is_valid()) continue;

    if (pos * m_LayerStep >= m_scroll)
      layer_icon->draw(context, get_layer_coords(pos), icon_size);
    else if ((pos + 1) * m_LayerStep >= m_scroll)
    {
      const int visible_pixels = std::min(32, static_cast<int>(
        static_cast<float>(m_LayerStep - (m_scroll - pos * m_LayerStep)) * 32.f / icon_size));
      layer_icon->draw(context, get_layer_coords(pos), visible_pixels, icon_size);
    }
  ...
  if (pos * m_LayerStep >= m_scroll)
    m_add_icon.draw(context, get_layer_coords(pos), icon_size);
  else if ((pos + 1) * m_LayerStep >= m_scroll)
  {
    const int visible_pixels = std::min(32, static_cast<int>(
      static_cast<float>(m_LayerStep - (m_scroll - pos * m_LayerStep)) * 32.f / icon_size));
    m_add_icon.draw(context, get_layer_coords(pos), visible_pixels, icon_size);
  }
}
...
EditorLayersWidget::update(float dt_sec)
{
  ...
  else if (m_scroll_speed > 0 && m_scroll < (static_cast<int>(m_layer_icons.size()) - 1) * m_LayerStep)
  {
    m_scroll += 5;
  }
}
...
EditorLayersWidget::on_mouse_button_down(const SDL_MouseButtonEvent& button)
{
  if (button.button == SDL_BUTTON_LEFT)
  {
    switch (m_hovered_item)
    {
      ...
      case HoveredItem::LAYERS:
        if (button.which == SDL_TOUCH_MOUSEID && m_hovered_layer < m_layer_icons.size())
        {
          LayerObject* layer = m_layer_icons[m_hovered_layer]->get_layer();
          MenuManager::instance().push_menu(std::make_unique<ObjectMenu>(layer));
          m_editor.select_object(layer);
          return true;
        }
        ...
        if (m_hovered_layer == m_layer_icons.size()) // Add layer button
        {
          ...
          if (m_add_layer_box_visible)
          {
            ...
            m_add_layer_box->set_rect(Rectf(coords - Vector(static_cast<float>(m_RowHeight),
                                                            std::max(m_add_layer_box->get_tiles_height(),
                                                                     static_cast<float>(m_RowHeight))),
                                            coords + Vector(static_cast<float>(m_RowHeight) * 2.f, 0.f)));
          }
        }
...
EditorLayersWidget::on_mouse_motion(const SDL_MouseMotionEvent& motion)
{
  ...
  if (x <= static_cast<float>(m_sector_text_width)) {
    ...
    // Scrolling
    if (x < static_cast<float>(m_sector_text_width + m_RowHeight))
    {
      m_scroll_speed = -1;
      m_add_layer_box_visible = false;
    }
    else if (x > static_cast<float>(m_Width - m_RowHeight))
    {
      m_scroll_speed = 1;
      m_add_layer_box_visible = false;
    }
...
EditorLayersWidget::on_mouse_wheel(const SDL_MouseWheelEvent& wheel)
{
  if (m_has_mouse_focus)
  {
    ...
    {
      auto last_item_position = (static_cast<int>(m_layer_icons.size()) - 1) * m_LayerStep;
...
{
  const int margin = editor_layer_margin();
  m_RowHeight = editor_layer_row_height();
  m_LayerStep = m_RowHeight + 3;
  m_Left = margin;
  m_Xpos = m_Left + m_RowHeight;
  m_Right = SCREEN_WIDTH - margin - editor_layer_toolbox_width();
  m_Ypos = SCREEN_HEIGHT - margin - m_RowHeight;
  m_Width = std::max(0, m_Right - m_Xpos);
}
...
{
  return Vector(static_cast<float>(pos * m_LayerStep + m_Xpos + m_sector_text_width - m_scroll),
...
{
  return static_cast<int>((coords.x - static_cast<float>(m_Xpos - m_scroll) - static_cast<float>(m_sector_text_width)) / static_cast<float>(m_LayerStep));
}
