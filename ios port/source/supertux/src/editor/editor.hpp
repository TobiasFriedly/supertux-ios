...
private:
  ...
  bool handle_touch_event(const SDL_TouchFingerEvent& touch, Uint32 finger_event_type);
  bool dispatch_touch_mouse_event(const SDL_TouchFingerEvent& touch, Uint32 mouse_event_type,
                                  bool editor_widgets_only = false);
  Vector touch_to_logical(const SDL_TouchFingerEvent& touch) const;
  bool is_touch_on_editor_widget(const Vector& pos) const;
  bool is_touch_on_toolbox_tilebox(const Vector& pos) const;
...
private:
  ...
  SDL_FingerID m_touch_finger;
  Uint32 m_touch_start_ticks;
  Vector m_touch_start_pos;
  Vector m_touch_last_pos;
  bool m_touch_mouse_down_sent;
  std::unordered_map<SDL_FingerID, Vector> m_touch_points;
  float m_touch_last_pinch_distance;
  Vector m_touch_last_pinch_center;
