...
static const float CAMERA_MAX_ZOOM = 8.0f;
...
float
editor_work_area_right()
{
  return static_cast<float>(SCREEN_WIDTH) - editor_ios_margin() - editor_right_toolbox_width();
}

float
editor_work_area_bottom()
{
  return static_cast<float>(SCREEN_HEIGHT) - editor_ios_margin() - editor_bottom_bar_height();
}

bool
is_touch_mouse_button_event(Uint32 event_type)
{
  return event_type == SDL_MOUSEBUTTONDOWN || event_type == SDL_MOUSEBUTTONUP;
}

Rectf
get_property_touch_rect(const InterfaceControl& control)
{
  Rectf touch_rect = control.get_rect();
  if (control.m_label)
  {
    const Rectf label_rect = control.m_label->get_rect();
    touch_rect = Rectf(std::min(touch_rect.get_left(), label_rect.get_left()),
                       std::min(touch_rect.get_top(), label_rect.get_top()),
                       std::max(touch_rect.get_right(), label_rect.get_right()),
                       std::max(touch_rect.get_bottom(), label_rect.get_bottom()));
  }

#ifdef SUPERTUX_IOS
  return touch_rect.grown(Vector(14.f, 8.f));
#else
  return touch_rect;
#endif
}
...
InterfaceControl*
find_property_touch_control(const std::vector<std::unique_ptr<InterfaceControl>>& controls,
                            const Vector& pos)
{
  InterfaceControl* best_control = nullptr;
  float best_distance = std::numeric_limits<float>::max();

  for (const auto& control : controls)
  {
    const Rectf touch_rect = get_property_touch_rect(*control);
    if (!touch_rect.contains(pos))
      continue;

    const float distance = std::abs(pos.y - touch_rect.get_middle().y);
    if (distance < best_distance)
    {
      best_control = control.get();
...
void
retarget_mouse_event_to_control(SDL_Event& event, const InterfaceControl& control)
{
  const Viewport& viewport = VideoSystem::current()->get_viewport();
  const Rect viewport_rect = viewport.get_rect();
  const Vector viewport_scale = viewport.get_scale();
  const Vector control_center = control.get_rect().get_middle();
  const int x = static_cast<int>(static_cast<float>(viewport_rect.left) +
                                 control_center.x * viewport_scale.x);
  const int y = static_cast<int>(static_cast<float>(viewport_rect.top) +
                                 control_center.y * viewport_scale.y);
  ...
  else if (is_touch_mouse_button_event(event.type))
  {
    event.button.x = x;
    event.button.y = y;
  }
...
Editor::Editor() :
  ...
  m_touch_start_pos(0.f, 0.f),
  m_touch_last_pos(0.f, 0.f),
  m_touch_mouse_down_sent(false),
  m_touch_points(),
  m_touch_last_pinch_distance(0.f),
  m_touch_last_pinch_center(0.f, 0.f),
...
Editor::draw(Compositor& compositor)
{
  ...
  if (m_levelloaded)
  {
    ...
    const float margin = editor_ios_margin();
    const float work_area_right = editor_work_area_right();
    const float work_area_bottom = editor_work_area_bottom();
    Rectf border_rect = Rectf{work_area_right - LINE_THICKNESS, margin,
                              work_area_right, work_area_bottom};
    ...
    Rectf layers_rect = Rectf{margin, work_area_bottom - LINE_THICKNESS,
                              work_area_right, work_area_bottom};
  ...
  if (m_new_scale != 0.f)
  {
    ...
    if (m_new_scale >= CAMERA_MIN_ZOOM && m_new_scale <= CAMERA_MAX_ZOOM)
    {
      ...
      if (zooming_in && !m_key_zoomed && !g_config->editor_zoom_centered)
        camera.move((m_mouse_pos - Vector(editor_work_area_right(),
                                          editor_work_area_bottom()) / 2.f) / CAMERA_ZOOM_FOCUS_PROGRESSION);
...
Editor::set_level(std::unique_ptr<Level> level, bool reset)
{
  ...
  if (m_sector != nullptr)
  {
    ...
    if (reset) {
      m_sector->get_camera().set_scale(editor_default_camera_scale());
    } else {
...
bool
Editor::dispatch_touch_mouse_event(const SDL_TouchFingerEvent& touch, Uint32 mouse_event_type,
                                   bool editor_widgets_only)
{
  if (!VideoSystem::current())
    return false;

  const Size window_size = VideoSystem::current()->get_window_size();
  const int x = std::max(0, std::min(window_size.width - 1,
                                     static_cast<int>(touch.x * static_cast<float>(window_size.width))));
  const int y = std::max(0, std::min(window_size.height - 1,
                                     static_cast<int>(touch.y * static_cast<float>(window_size.height))));

  SDL_Event mouse_event;
  SDL_memset(&mouse_event, 0, sizeof(mouse_event));
  mouse_event.type = mouse_event_type;

  if (mouse_event_type == SDL_MOUSEMOTION)
  {
    mouse_event.motion.windowID = touch.windowID;
    mouse_event.motion.which = SDL_TOUCH_MOUSEID;
    mouse_event.motion.state = m_touch_mouse_down_sent ? SDL_BUTTON_LMASK : 0;
    mouse_event.motion.x = x;
    mouse_event.motion.y = y;
    mouse_event.motion.xrel = static_cast<int>(touch.dx * static_cast<float>(window_size.width));
    mouse_event.motion.yrel = static_cast<int>(touch.dy * static_cast<float>(window_size.height));
  }
  else
  {
    mouse_event.button.windowID = touch.windowID;
    mouse_event.button.which = SDL_TOUCH_MOUSEID;
    mouse_event.button.button = SDL_BUTTON_LEFT;
    mouse_event.button.state = mouse_event_type == SDL_MOUSEBUTTONDOWN ? SDL_PRESSED : SDL_RELEASED;
    mouse_event.button.clicks = 1;
    mouse_event.button.x = x;
    mouse_event.button.y = y;
  }

  MouseCursor::current()->set_pos(x, y);
  auto make_hover_event = [&]() {
    SDL_Event hover_event;
    SDL_memset(&hover_event, 0, sizeof(hover_event));
    hover_event.type = SDL_MOUSEMOTION;
    hover_event.motion.windowID = touch.windowID;
    hover_event.motion.which = SDL_TOUCH_MOUSEID;
    hover_event.motion.state = 0;
    hover_event.motion.x = x;
    hover_event.motion.y = y;
    hover_event.motion.xrel = 0;
    hover_event.motion.yrel = 0;
    return hover_event;
  };

  if (editor_widgets_only)
  {
    m_mouse_pos = VideoSystem::current()->get_viewport().to_logical(x, y);
    InterfaceControl* touched_control = is_touch_mouse_button_event(mouse_event_type) ?
      find_property_touch_control(m_controls, m_mouse_pos) :
      nullptr;

    if (mouse_event_type == SDL_MOUSEBUTTONDOWN)
    {
      const SDL_Event hover_event = make_hover_event();

      for (const auto& control : m_controls)
      {
        SDL_Event control_event = hover_event;
        if (control.get() == touched_control)
          retarget_mouse_event_to_control(control_event, *control);
        control->event(control_event);
      }
      ...
      try
      {
        ...
        m_toolbar_widget->event(hover_event);
        m_toolbox_widget->event(hover_event);
        m_layers_widget->event(hover_event);
      }
      catch (const std::exception& err)
      {
        log_warning << "error while priming Editor touch widget hover: " << err.what() << std::endl;
      }
    }

    for (const auto& control : m_controls)
    {
      SDL_Event control_event = mouse_event;
      if (control.get() == touched_control)
      {
        retarget_mouse_event_to_control(control_event, *control);
      }

      if (control->event(control_event))
        return true;
    }
    ...
    try
    {
      ...
      if (m_toolbar_widget->event(mouse_event))
        return true;
      if (m_toolbox_widget->event(mouse_event))
        return true;
      if (m_layers_widget->event(mouse_event))
        return true;
    }
    catch (const std::exception& err)
    {
      log_warning << "error while processing Editor touch widget event: " << err.what() << std::endl;
    }
...
Vector
Editor::touch_to_logical(const SDL_TouchFingerEvent& touch) const
{
  if (!VideoSystem::current())
  ...
  return VideoSystem::current()->get_viewport().to_logical(x, y);
}

bool
Editor::is_touch_on_editor_widget(const Vector& pos) const
{
  const float margin = editor_ios_margin();
  const float work_area_right = editor_work_area_right();
  const float work_area_bottom = editor_work_area_bottom();
#ifdef SUPERTUX_IOS
  const float touch_slop = 72.f;
  const float top_widget_height = 120.f;
#else
  const float touch_slop = 24.f;
  const float top_widget_height = 56.f;
#endif

  if (pos.y <= margin + top_widget_height)
    return true;
  if (pos.x >= work_area_right - touch_slop)
    return true;
  if (pos.y >= work_area_bottom - touch_slop)
    return true;
  if (!m_controls.empty() && Rectf(0.f, 32.f, 200.f, SCREEN_HEIGHT - 32.f).contains(pos))
    return true;
...
bool
Editor::is_touch_on_toolbox_tilebox(const Vector& pos) const
{
  return m_toolbox_widget && m_toolbox_widget->get_tilebox().get_rect().contains(pos);
}

bool
Editor::handle_touch_event(const SDL_TouchFingerEvent& touch, Uint32 finger_event_type)
{
  if (!VideoSystem::current())
    return false;

  const Vector pos = touch_to_logical(touch);

  if (finger_event_type == SDL_FINGERDOWN)
  {
    if (m_touch_mode == TouchMode::DIRECT)
    {
      if (is_touch_on_toolbox_tilebox(m_touch_start_pos) &&
          is_touch_on_toolbox_tilebox(pos))
      {
        m_touch_points[m_touch_finger] = m_touch_last_pos;
        m_touch_points[touch.fingerId] = pos;

        auto it = m_touch_points.begin();
        const Vector first = it->second;
        ++it;
        const Vector second = it->second;

        m_touch_mode = TouchMode::TOOLBOX_SCROLL;
        m_touch_mouse_down_sent = false;
        m_touch_last_pinch_center = (first + second) / 2.f;
        m_touch_last_pinch_distance = glm::length(second - first);
        m_toolbox_widget->get_tilebox().cancel_touch_selection();
      }
      return true;
    }

    if (is_touch_on_editor_widget(pos))
    {
      if (m_touch_mode == TouchMode::NONE)
      {
        m_touch_mode = TouchMode::DIRECT;
        m_touch_finger = touch.fingerId;
        m_touch_start_pos = pos;
        m_touch_last_pos = pos;
        m_touch_points[touch.fingerId] = pos;
        m_touch_mouse_down_sent = true;
        dispatch_touch_mouse_event(touch, SDL_MOUSEBUTTONDOWN, true);
      }
      return true;
    }
    ...
    if (m_touch_mode == TouchMode::NONE)
    {
      m_touch_mode = TouchMode::PLACE;
      m_touch_finger = touch.fingerId;
      m_touch_start_pos = pos;
      m_touch_last_pos = pos;
      m_touch_mouse_down_sent = true;
      dispatch_touch_mouse_event(touch, SDL_MOUSEBUTTONDOWN);
    }
    else if (m_touch_mode == TouchMode::PLACE && m_touch_points.size() >= 2)
    {
      if (m_touch_mouse_down_sent)
        dispatch_touch_mouse_event(touch, SDL_MOUSEBUTTONUP);
      ...
      m_touch_mode = TouchMode::TWO_FINGER;
      m_touch_mouse_down_sent = false;
      m_touch_last_pinch_center = (first + second) / 2.f;
      m_touch_last_pinch_distance = glm::length(second - first);
    }
  ...
  if (finger_event_type == SDL_FINGERMOTION)
  {
    if (m_touch_mode == TouchMode::DIRECT)
    {
      if (touch.fingerId == m_touch_finger)
      {
        m_touch_points[touch.fingerId] = pos;
        m_touch_last_pos = pos;
        dispatch_touch_mouse_event(touch, SDL_MOUSEMOTION, true);
      }
      return true;
    }

    auto touch_it = m_touch_points.find(touch.fingerId);
    if (touch_it == m_touch_points.end())
      return true;

    touch_it->second = pos;

    if (m_touch_mode == TouchMode::PLACE)
    {
      if (touch.fingerId == m_touch_finger)
      {
        m_touch_last_pos = pos;
        dispatch_touch_mouse_event(touch, SDL_MOUSEMOTION);
      }
      return true;
    }

    if (m_touch_mode == TouchMode::TOOLBOX_SCROLL && m_touch_points.size() >= 2)
    {
      auto it = m_touch_points.begin();
      const Vector first = it->second;
      ++it;
      const Vector second = it->second;
      const Vector center = (first + second) / 2.f;
      const Vector delta = center - m_touch_last_pinch_center;

      m_toolbox_widget->get_tilebox().scroll_by(delta.y);
      m_touch_last_pinch_center = center;
      m_touch_last_pinch_distance = glm::length(second - first);
      return true;
    }

    if (m_touch_mode == TouchMode::TWO_FINGER && m_touch_points.size() >= 2)
    {
      auto it = m_touch_points.begin();
      const Vector first = it->second;
      ++it;
      const Vector second = it->second;
      const Vector center = (first + second) / 2.f;
      const float distance = glm::length(second - first);
      const Vector delta = center - m_touch_last_pinch_center;

      scroll(Vector(-delta.x, -delta.y));

      if (m_touch_last_pinch_distance > 4.f && distance > 4.f)
      {
        const float ratio = distance / m_touch_last_pinch_distance;
        if (std::abs(ratio - 1.f) > 0.01f)
        {
          m_mouse_pos = center;
          const float base_scale = (m_new_scale != 0.f) ?
            m_new_scale :
            m_sector->get_camera().get_current_scale();
          m_new_scale = std::clamp(base_scale * ratio,
                                   CAMERA_MIN_ZOOM, CAMERA_MAX_ZOOM);
        }
      }

      m_touch_last_pinch_center = center;
      m_touch_last_pinch_distance = distance;
      return true;
    }
  ...
  if (finger_event_type == SDL_FINGERUP)
  {
    if (m_touch_mode == TouchMode::DIRECT)
    {
      if (touch.fingerId == m_touch_finger)
      {
        dispatch_touch_mouse_event(touch, SDL_MOUSEBUTTONUP, true);
        reset_touch_gesture();
      }
      return true;
    }
    ...
    if (m_touch_mode == TouchMode::PLACE)
    {
      if (touch.fingerId == m_touch_finger && m_touch_mouse_down_sent)
        dispatch_touch_mouse_event(touch, SDL_MOUSEBUTTONUP);
    ...
    if (m_touch_mode == TouchMode::TOOLBOX_SCROLL)
    {
      m_touch_points.erase(touch.fingerId);
      if (m_touch_points.size() < 2)
      {
        reset_touch_gesture();
      }
      else
    ...
    if (m_touch_mode == TouchMode::TWO_FINGER)
    {
      m_touch_points.erase(touch.fingerId);
      if (m_touch_points.size() < 2)
      {
...
void
Editor::reset_touch_gesture()
{
  m_touch_mode = TouchMode::NONE;
  m_touch_finger = 0;
  m_touch_start_ticks = 0;
  m_touch_start_pos = Vector(0.f, 0.f);
  m_touch_last_pos = Vector(0.f, 0.f);
  m_touch_mouse_down_sent = false;
  m_touch_points.clear();
  m_touch_last_pinch_distance = 0.f;
  m_touch_last_pinch_center = Vector(0.f, 0.f);
}
...
Editor::event(const SDL_Event& ev)
{
  if (!m_enabled || !m_levelloaded ||
      MenuManager::current()->is_active() || MenuManager::current()->has_dialog())
  {
    reset_touch_gesture();
    return;
  }
  ...
  {
    handle_touch_event(ev.tfinger, ev.type);
    return;
  }
        ...
        else if (m_ctrl_pressed)
        {
          switch (ev.key.keysym.sym)
          {
            ...
            case SDLK_d: // Reset zoom
              m_new_scale = editor_default_camera_scale();
              break;
            default:
              break;
          }
