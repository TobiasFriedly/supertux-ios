...
float
editor_toolbox_width()
{
  return editor_tool_size() * 4.f;
}

float
editor_toolbox_header_height()
{
  return editor_tool_size() * 3.f;
}
...
EditorToolboxWidget::draw(DrawingContext& context)
{
  ...
  context.color().draw_filled_rect(Rectf(Vector(m_pos_x, margin),
                                         Vector(right, margin + editor_toolbox_header_height())),
...
EditorToolboxWidget::on_mouse_motion(const SDL_MouseMotionEvent& motion)
{
  ...
  const float tool_size = editor_tool_size();
...
EditorToolboxWidget::on_window_resize()
{
  ...
  m_pos_x = right - editor_toolbox_width();
  m_tilebox->set_rect(Rectf(Vector(m_pos_x, margin + editor_toolbox_header_height()),
                            Vector(right, static_cast<float>(SCREEN_HEIGHT) - margin - tool_size)));

  m_rubber->set_size(tool_size);
  m_select_mode->set_size(tool_size);
  m_node_marker_mode->set_size(tool_size);
  m_move_mode->set_size(tool_size);
  m_undo_mode->set_size(tool_size);

  const float tool_y = margin + tool_size * 2.f;
  m_rubber->m_pos           = Vector(m_pos_x                  , tool_y);
  m_select_mode->m_pos      = Vector(m_pos_x + tool_size      , tool_y);
  m_node_marker_mode->m_pos = Vector(m_pos_x + tool_size      , tool_y);
  m_move_mode->m_pos        = Vector(m_pos_x + tool_size * 2.f, tool_y);
  m_undo_mode->m_pos        = Vector(m_pos_x + tool_size * 3.f, tool_y);
...
{
  const float tool_size = editor_tool_size();
  return Vector(static_cast<float>(pos % 4) * tool_size + m_pos_x,
                static_cast<float>(pos / 4) * tool_size + editor_ios_margin() + tool_size * 2.f);
}
...
{
  const float tool_size = editor_tool_size();
  int x = static_cast<int>((coords.x - m_pos_x) / tool_size);
  int y = static_cast<int>((coords.y - editor_ios_margin() - tool_size * 2.f) / tool_size);
  return y * 4 + x;
}
...
EditorToolboxWidget::get_rect_from_hovered_item(HoveredItem item) const
{
  ...
  {
    case HoveredItem::TILEGROUP:
      return Rectf(Vector(m_pos_x, editor_ios_margin()),
                   Vector(static_cast<float>(SCREEN_WIDTH) - editor_ios_margin(),
                          editor_ios_margin() + editor_tool_size()));

    case HoveredItem::OBJECTS:
      return Rectf(Vector(m_pos_x, editor_ios_margin() + editor_tool_size()),
                   Vector(static_cast<float>(SCREEN_WIDTH) - editor_ios_margin(),
                          editor_ios_margin() + editor_tool_size() * 2.f));

    case HoveredItem::TOOL:
    {
      auto coords = get_tool_coords(m_hovered_tool);
      return Rectf(coords, coords + Vector(editor_tool_size(), editor_tool_size()));
    }
