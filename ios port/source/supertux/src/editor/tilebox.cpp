...
int
editor_tilebox_columns()
{
  return 4;
}

float
editor_tilebox_scrollbar_width()
{
#ifdef SUPERTUX_IOS
  return 18.f;
#else
...
{
  int pos = -1;
  const int columns = editor_tilebox_columns();
  const float cell_size = editor_tilebox_cell_size();
  for (auto& tile_ID : m_active_tilegroup->tiles)
  {
    pos++;
    if (pos / columns < static_cast<int>(m_scroll_progress / cell_size))
      continue;

    auto position = get_tile_coords(pos, false);
    const auto surface = Tile::draw_editor_images ?
      m_editor.get_tileset()->get(tile_ID).get_current_editor_surface() :
      m_editor.get_tileset()->get(tile_ID).get_current_surface();

    if (surface)
    {
      context.color().draw_surface_scaled(surface,
                                          Rectf(position, position + Vector(cell_size, cell_size)),
                                          LAYER_GUI - 9);
    }
    ...
    if (g_config->developer_mode && (m_active_tilegroup->developers_group || g_debug.show_toolbox_tile_ids) && tile_ID != 0)
    {
      ...
      context.color().draw_text(Resources::console_font, std::to_string(tile_ID),
                                position + Vector(cell_size / 2.f, cell_size / 2.f),
                                ALIGN_CENTER, LAYER_GUI - 9, Color::WHITE);
    }
  }
}
...
EditorTilebox::draw_objectgroup(DrawingContext& context)
{
  ...
  for (auto& icon : m_active_objectgroup->get_icons())
  {
    ...
    icon.draw(context, get_tile_coords(pos, false), cell_size);
  }
}
...
{
  const float cell_size = editor_tilebox_cell_size();
  const float tile_scroll_progress = (rounded ? floorf(m_scroll_progress / cell_size) : m_scroll_progress / cell_size);

  Vector drag_start = m_drag_start - Vector(0.f, tile_scroll_progress);
  Vector drag_end = Vector(static_cast<float>(m_hovered_tile % editor_tilebox_columns()),
                           static_cast<float>(m_hovered_tile / editor_tilebox_columns()) - tile_scroll_progress); // NOLINT
...
EditorTilebox::selection_draw_rect() const
{
  ...
  const float cell_size = editor_tilebox_cell_size();
  select.set_p1((select.p1() * cell_size) + m_rect.p1());
  select.set_p2((select.p2() * cell_size) + m_rect.p1());
...
EditorTilebox::update_selection()
{
  ...
  for (int y = static_cast<int>(select.get_top()); y <= static_cast<int>(select.get_bottom()); y++)
  {
    ...
    {
      int tile_pos = (y + static_cast<int>(m_scroll_progress / editor_tilebox_cell_size())) * editor_tilebox_columns() + x;
    ...
    if (m_hovered_item == HoveredItem::TILE)
    {
      switch (m_input_type)
      {
        case InputType::TILE:
          {
            m_dragging = true;
            m_drag_start = Vector(static_cast<float>(m_hovered_tile % editor_tilebox_columns()),
                                  static_cast<float>(m_hovered_tile / editor_tilebox_columns())); // NOLINT
...
EditorTilebox::on_window_resize()
{
  ...
  m_scrollbar->set_rect(Rectf(Vector(m_rect.get_right() - editor_tilebox_scrollbar_width(), m_rect.get_top()), m_rect.p2()));
}

void
EditorTilebox::scroll_by(float logical_delta_y)
{
  if (!m_scrollbar->is_active())
    return;

  const float max_scroll = std::max(0.f, get_tiles_height() - m_rect.get_height());
  m_scroll_progress = std::clamp(m_scroll_progress - logical_delta_y, 0.f, max_scroll);
  m_dragging = false;
  m_object_tip->set_visible(false);
...
void
EditorTilebox::cancel_touch_selection()
{
  SDL_MouseButtonEvent release_event;
  SDL_memset(&release_event, 0, sizeof(release_event));
  release_event.button = SDL_BUTTON_LEFT;
  m_scrollbar->on_mouse_button_up(release_event);
...
EditorTilebox::get_tile_coords(int pos, bool relative) const
{
  const float cell_size = editor_tilebox_cell_size();
  const int columns = editor_tilebox_columns();
  return Vector(static_cast<float>(pos % columns) * cell_size + (relative ? m_rect.get_left() : 0),
                static_cast<float>(pos / columns) * cell_size + (relative ? m_rect.get_top() : 0) - m_scroll_progress);
}
...
EditorTilebox::get_tile_pos(const Vector& coords) const
{
  const float cell_size = editor_tilebox_cell_size();
  const int columns = editor_tilebox_columns();
  int x = static_cast<int>((coords.x - m_rect.get_left()) / cell_size);
  int y = static_cast<int>((coords.y - m_rect.get_top() + m_scroll_progress) / cell_size);
  return y * columns + x;
}
...
EditorTilebox::get_tile_rect() const
{
  ...
  const float cell_size = editor_tilebox_cell_size();
  Rectf rect(coords, coords + Vector(cell_size, cell_size));
...
EditorTilebox::get_tiles_height() const
{
  ...
  {
    case InputType::TILE:
      return ceilf(static_cast<float>(m_active_tilegroup->tiles.size()) /
                   static_cast<float>(editor_tilebox_columns())) * editor_tilebox_cell_size();

    case InputType::OBJECT:
      return ceilf(static_cast<float>(m_active_objectgroup->get_icons().size()) /
                   static_cast<float>(editor_tilebox_columns())) * editor_tilebox_cell_size();
