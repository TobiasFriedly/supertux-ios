...
void
EditorToolbarWidget::relayout()
{
  const float button_size = editor_toolbar_button_size();
  const float margin = editor_toolbar_margin();
  ...
  m_widgets_width = static_cast<float>(visible_count) * button_size;
#ifdef SUPERTUX_MOBILE
  m_origin = Vector(std::max(margin, (static_cast<float>(SCREEN_WIDTH) - m_widgets_width) / 2.f), margin);
#else
  m_origin = Vector(0.f, 0.f);
#endif
  ...
  for (const auto& toolbar_button : m_widgets)
  {
    ...
    toolbar_button->set_size(Sizef(button_size, button_size));
    toolbar_button->set_position(m_origin + Vector(button_size * static_cast<float>(visible_index), 0.f));
    ++visible_index;
  }
}
...
EditorToolbarWidget::draw(DrawingContext& context)
{
  ...
  context.color().draw_filled_rect(
    {m_origin.x - g_config->menuroundness,
     m_origin.y - g_config->menuroundness,
     m_origin.x + m_widgets_width + m_widgets_width_offset,
     m_origin.y + editor_toolbar_button_size()},
