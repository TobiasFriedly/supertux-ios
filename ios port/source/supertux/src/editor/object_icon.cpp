...
void
ObjectIcon::draw(DrawingContext& context, const Vector& pos, float size)
{
  const float scale = size / 32.f;
  const Vector offset = m_offset * scale;
  context.color().draw_surface_scaled(m_surface,
                                      Rectf(
                                        pos + offset,
                                        pos + Vector(size, size) - offset
                                      ),
                                      LAYER_GUI - 9);
}
...
void
ObjectIcon::draw(DrawingContext& context, const Vector& pos, int pixels_shown, float size)
{
  const float scale = size / 32.f;
  const Vector offset = m_offset * scale;
  const float shown = static_cast<float>(pixels_shown) * scale;
  ...
  context.color().draw_surface_scaled(cropped_surface,
                                      Rectf(
                                        pos + Vector(size - shown, 0) + offset,
                                        pos + Vector(size, size) - offset
                                      ),
                                      LAYER_GUI - 9);
}
