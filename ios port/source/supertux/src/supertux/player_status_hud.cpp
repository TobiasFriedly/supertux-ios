...
PlayerStatusHUD::draw(DrawingContext& context)
{
  ...
  float hudpos = hud_top_inset() + 1.0f;
  ...
  if (stat_surface)
  {
    context.color().draw_surface(stat_surface,
                                Vector(context.get_width() - hud_right_inset() - static_cast<float>(stat_surface->get_width()) - Resources::fixed_font->get_text_width(stat_text),
                                       hudpos),
                                LAYER_HUD);
  }
  ...
  context.color().draw_text(Resources::fixed_font,
                            stat_text,
                            Vector(static_cast<float>(context.get_width()) - hud_right_inset() - Resources::fixed_font->get_text_width(stat_text),
                                  hudpos + 13.f),
                            ALIGN_LEFT,
                            LAYER_HUD,
  ...
  if (m_player_status.is_item_pocket_allowed())
  {
    for (int i = 0; i < InputManager::current()->get_num_users(); i++)
    {
      ...
      Vector pos(hud_left_inset(), hud_top_inset() + ypos);
