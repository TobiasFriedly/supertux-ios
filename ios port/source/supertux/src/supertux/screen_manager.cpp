...
bool
editor_canvas_owns_touch()
{
#ifdef SUPERTUX_MOBILE
...
bool
has_connected_physical_controller(InputManager& input_manager)
{
  return !input_manager.game_controller_manager->get_controller_mapping().empty() ||
         !input_manager.joystick_manager->get_joystick_mapping().empty();
}

void
update_touch_controls_for_controller(InputManager& input_manager)
{
#ifdef SUPERTUX_MOBILE
  if (!g_config->mobile_controls)
    return;

  const bool controller_connected = has_connected_physical_controller(input_manager);

  if (!controller_connected)
  {
    if (g_config->touch_controls_auto_hidden_by_controller || !g_config->touch_controls_visibility_manually_set)
    {
      const bool changed = !g_config->touch_controls_visible || g_config->touch_controls_auto_hidden_by_controller;
      g_config->touch_controls_visible = true;
      g_config->touch_controls_user_override = g_config->touch_controls_visibility_manually_set;
      g_config->touch_controls_auto_hidden_by_controller = false;
      if (changed)
        g_config->save();
    }
    return;
  ...
  if (g_config->touch_controls_visibility_manually_set || g_config->touch_controls_user_override)
    return;

  if (g_config->touch_controls_visible)
  {
    g_config->touch_controls_visible = false;
    g_config->touch_controls_auto_hidden_by_controller = true;
    g_config->save();
  }
#else
  (void)input_manager;
...
ScreenManager::draw(Compositor& compositor, FPS_Stats& fps_statistics)
{
  ...
  if (g_config->mobile_controls)
  {
    const bool editor_canvas_touch = editor_canvas_owns_touch();
    m_mobile_controller.draw(context, !editor_canvas_touch, editor_canvas_touch);
  }
...
ScreenManager::update_gamelogic(float dt_sec)
{
  ...
  update_touch_controls_for_controller(m_input_manager);

  if (g_config->mobile_controls)
  {
    const bool editor_canvas_touch = editor_canvas_owns_touch();
    m_mobile_controller.update(!editor_canvas_touch, editor_canvas_touch);
...
ScreenManager::process_events()
{
  ...
  while (SDL_PollEvent(&event))
  {
    ...
    bool event_consumed_by_mobile_controller = false;
    const bool editor_canvas_touch = editor_canvas_owns_touch();
    const bool movement_controls_enabled = !editor_canvas_touch;
    const bool editor_pause_only = editor_canvas_touch;
    ...
    switch (event.type)
    {
      case SDL_FINGERDOWN:
      {
        ...
        if (m_mobile_controller.process_finger_down_event(event.tfinger, movement_controls_enabled, editor_pause_only))
        {
          event_consumed_by_mobile_controller = true;
          break; // Event was processed by touch controls, do not generate mouse event
        }

        if (editor_canvas_touch)
          break;
      ...
      case SDL_FINGERUP:
      {
        ...
        if (editor_canvas_touch)
        {
          if (m_mobile_controller.process_finger_up_event(event.tfinger, movement_controls_enabled, editor_pause_only))
            event_consumed_by_mobile_controller = true;
          break;
        }
      ...
      case SDL_FINGERMOTION:
      {
        ...
        if (m_mobile_controller.process_finger_motion_event(event.tfinger, movement_controls_enabled, editor_pause_only))
        {
          event_consumed_by_mobile_controller = true;
          break; // Event was processed by touch controls, do not generate mouse event
        }
    ...
    if (event_consumed_by_mobile_controller)
      continue;
