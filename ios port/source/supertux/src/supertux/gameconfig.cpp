...
Config::Config() :
...
#if defined(SUPERTUX_IOS)
  touch_floating_joystick(true),
#else
  touch_floating_joystick(false),
#endif
  mobile_controls(false),
  m_mobile_controls_scale(1.3f),
  touch_controls_visible(true),
  touch_controls_user_override(false),
  touch_controls_visibility_manually_set(false),
  touch_controls_auto_hidden_by_controller(false),
  addons(),
  ...
  ios_network_migration_done(false),
...
Config::load()
{
  ...
  config_mapping.get("unlock_all_levels", unlock_all_levels);
  config_mapping.get("music_favorites", music_favorites);
...
#endif
#if defined(SUPERTUX_IOS) && defined(NETWORKING)
  config_mapping.get("ios_network_migration_done", ios_network_migration_done);
  if (!ios_network_migration_done)
  {
    disable_network = false;
    ios_network_migration_done = true;
  }
#endif
  ...
  if (config_mapping.get("control", config_control_mapping))
  {
    ...
    config_control_mapping->get("touch_floating_joystick", touch_floating_joystick);
    ...
    config_control_mapping->get("touch_controls_user_override", touch_controls_user_override);
    config_control_mapping->get("touch_controls_visibility_manually_set", touch_controls_visibility_manually_set);
    config_control_mapping->get("touch_controls_auto_hidden_by_controller", touch_controls_auto_hidden_by_controller);
...
Config::save()
{
  ...
  writer.write("unlock_all_levels", unlock_all_levels);
  writer.write("music_favorites", music_favorites);
  ...
  writer.write("ios_network_migration_done", ios_network_migration_done);
  ...
  writer.start_list("control");
  {
    ...
    writer.write("touch_floating_joystick", touch_floating_joystick);
    writer.write("mobile_controls_scale", m_mobile_controls_scale);
    writer.write("touch_controls_visible", touch_controls_visible);
    writer.write("touch_controls_user_override", touch_controls_user_override);
    writer.write("touch_controls_visibility_manually_set", touch_controls_visibility_manually_set);
    writer.write("touch_controls_auto_hidden_by_controller", touch_controls_auto_hidden_by_controller);
