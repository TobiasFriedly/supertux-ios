  ...
  switch (m_type) // Insert label and menu items, appropriate for the chosen OptionsMenu type
  {
    ...
    case AUDIO: /** AUDIO */
    {
      ...
      if (SoundManager::current()->is_audio_enabled())
      {
        ...
        add_sound_volume();
        add_music_volume();
        add_submenu(_("Music Player"), MenuStorage::MUSIC_MENU)
          .set_help(_("Listen to the music tracks included with the game"));
      }
      else
      {
...
#else
      add_toggle(MNID_TOUCH_CONTROLS_VISIBLE, _("Show Touch Controls"), &g_config->touch_controls_visible)
        .set_help(_("If disabled, touch controls will still work, but the buttons will remain hidden."));

      add_toggle(-1, _("Floating Movement Joystick"), &g_config->touch_floating_joystick)
        .set_help(_("Use a floating joystick that starts where you touch the bottom-left movement area"));
    ...
    case ADVANCED: /** ADVANCED */
    {
      ...
      add_toggle(-1, _("Unlock All Levels for Testing"), &g_config->unlock_all_levels)
        .set_help(_("Temporarily makes locked worlds and levels available without changing saved progress"));
     ...
     defined(__OPENBSD) || defined(__NetBSD)) && !(defined(STEAM_BUILD) || defined(SUPERTUX_MOBILE) || defined(FLATPAK))
...
{
  m_magnifications.list.clear();
  m_magnifications.next = 0;
  ...
  m_magnifications.list.push_back("112.5%");
  m_magnifications.list.push_back("125%");
  m_magnifications.list.push_back("150%");
  m_magnifications.list.push_back("175%");
  ...
  m_magnifications.list.push_back("80%");
#endif
  m_magnifications.list.push_back("87.5%");
  m_magnifications.list.push_back("100%");
  m_magnifications.list.push_back("125%");
...
{
  m_mobile_control_scales.list.clear();
  m_mobile_control_scales.next = 0;
...
#ifdef SUPERTUX_IOS
        g_config->magnification = std::max(1.0f, g_config->magnification);
#endif
      }
      VideoSystem::current()->apply_config();
      ScreenManager::current()->on_window_resize();
...
#pragma GCC diagnostic pop
    ...
    case MNID_TOUCH_CONTROLS_VISIBLE:
      g_config->touch_controls_user_override = true;
      g_config->touch_controls_visibility_manually_set = true;
